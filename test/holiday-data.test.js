...
test('country options: None first, US second, Sweden present, no duplicate values', () => {
  assert.deepEqual(COUNTRY_OPTIONS[0], ['None', 'none']);
  assert.deepEqual(COUNTRY_OPTIONS[1], ['United States', 'US']);
  ...
  assert.equal(COUNTRY_OPTIONS.length, 34, 'expected None + US + 32 European');
});

test('region maps: only the six region-varying countries', () => {
  assert.deepEqual(Object.keys(REGION_OPTIONS).sort(), ['AT', 'CH', 'DE', 'ES', 'GB', 'US']);
});
